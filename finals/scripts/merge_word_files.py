...
def merge_word_files():
    ...
    if len(heading1_list) == expected_heading1_count:
        ...
        if len(heading1_list) < expected_heading1_count:
            print(f'   {expected_heading1_count - len(heading1_list)}個のHeading 1が不足しています')
        else:
            print(f'   {len(heading1_list) - expected_heading1_count}個のHeading 1が超過しています')
    
    print()
    print('次のステップ:')
    print('  1. Wordで目次を更新: 目次を右クリック → 「フィールドの更新」')
    print('  2. ページ番号を設定: 「ページ番号の書式設定」→ 開始番号を1に設定')
    print('  3. 最終確認: 誤字脱字、フォーマットの確認')
    print('  4. Amazon KDPに提出')
