...
def main():
    ...
    # v29から古い免責事項と奥付を削除
    # 段落102（免責事項の見出し）から最後まで削除
    print("✓ 古い免責事項と奥付を削除中...")
    
    # 免責事項の見出しを探す（段落103, インデックス102）
    disclaimer_index = None
    for i, para in enumerate(doc.paragraphs):
        if para.text.strip() == '免責事項':
            disclaimer_index = i
            print(f"  免責事項の見出しが見つかりました: 段落{i+1}")
            break
    
    if disclaimer_index is not None:
        # disclaimer_indexから最後まで削除
        paragraphs_to_remove = []
        for i in range(disclaimer_index, len(doc.paragraphs)):
            paragraphs_to_remove.append(doc.paragraphs[i])
        
        print(f"  削除する段落数: {len(paragraphs_to_remove)}")
        
        for paragraph in paragraphs_to_remove:
            p = paragraph._element
            p.getparent().remove(p)
        
        print(f"  削除後の総段落数: {len(doc.paragraphs)}")
    else:
        print("  警告: 免責事項の見出しが見つかりませんでした")
    ...
    print(f"  最終的な総段落数: {len(doc.paragraphs)}")
