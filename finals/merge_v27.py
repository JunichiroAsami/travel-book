#!/usr/bin/env python3
"""
v27のWord文書を結合するスクリプト（テーブルを正しく保持）
"""
...
def merge_docx_preserve_tables(files, output):
    """複数のWord文書を結合（テーブルを正しく保持）"""
...
output_file = 'complete_manuscript_v27_test.docx'

print("=== v27のWord文書を結合中（テーブル保持） ===")
merge_docx_preserve_tables(files, output_file)
...
for i, table in enumerate(doc.tables, 1):
    if table.rows:
        ...
        if col_count == 1:
            print(f"⚠️ テーブル{i}: 1列のみ（崩れている可能性）")
        elif i <= 5:
